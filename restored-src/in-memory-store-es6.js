// 还原后的内存存储模块代码 (模块ID: 78017) - ES6版本
import * as defaultSettings from './default-settings'; // 对应 n(22869)
import cloneDeep from './clone-deep'; // 对应 n(89138)
import { presistentData } from './persistent-data'; // 对应 n(33386)
import { LocalDb } from './local-db'; // 对应 n(84582)
...
export class InMemoryStore {
    ...
    getDefaultState() {
        return Object.freeze({
            // 交易员设置
            traderSettings: cloneDeep(defaultSettings.defaultSettings),

            // 交易员数据
            traderData: cloneDeep(defaultSettings.defaultTraderData),

            // 增强器数据
            enhancerData: cloneDeep(defaultSettings.defaultEnhancerData),

            // 增强器设置
            enhancerSettings: cloneDeep(defaultSettings.defaultEnhancerSetting),

            // 快捷键设置
            shortCutKeySettings: cloneDeep(defaultSettings.defaultShortcutKeySetting),
            ...
            // 阵容构建器设置
            squadBuilderSettings: cloneDeep(defaultSettings.defaultSquadBuilderSetting),

            // SBC求解器数据点（按角色分组）
            solverDataPointsPerPersona: cloneDeep(defaultSettings.defaultSBCSolverDataPoints)
        });
    }
    ...
    async init() {
        ...
        // 并行更新所有持久化数据
        const updatePromises = presistentData.map(key =>
